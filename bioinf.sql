...
--
-- Temporary view structure for view `viewer`
--
...
/*!50013 DEFINER=`guest`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `viewer` AS select concat(`modorg`.`genus`,' ',`modorg`.`species`) AS `Organism`,`class`.`name` AS `Classname`,`gene`.`acc` AS `Accesion`,`gene`.`len` AS `Length`,`gene`.`descr` AS `description` from ((`gene` join `modorg` on((`gene`.`mo_id` = `modorg`.`id`))) join `class` on((`modorg`.`class_id` = `class`.`id`))) */;
...
-- Dump completed on 2016-12-08 16:44:37
